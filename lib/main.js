...
function dtypes2signatures( dtypes, nin, nout ) {
	...
	if ( !isArrayLikeObject( dtypes ) ) {
		throw new TypeError( format( '0id2O', dtypes ) );
	}
	if ( !isNonNegativeInteger( nin ) ) {
		throw new TypeError( format( '0id3X', nin ) );
	}
	if ( !isNonNegativeInteger( nout ) ) {
		throw new TypeError( format( '0id2m', nout ) );
	}
	len = dtypes.length;
	if ( len === 0 ) {
		throw new RangeError( format('0id0a') );
	}
	N = nin + nout;
	if ( len%N !== 0 ) {
		throw new RangeError( format('0idD0') );
	}
	out = [];
